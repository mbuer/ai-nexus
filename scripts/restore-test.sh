...
cat "$latest" | podman exec -i ai-nexus-postgres     pg_restore -U postgres -d "$test_db" --no-owner
...
echo "Restore test passed. Recovery is performed with the PostgreSQL administrative role; Birdynator remains a runtime-only role."
