...
[[ "$security_state" == *'NoNewPrivs:'$'\t''1'* ]] && ok "PostgreSQL no-new-privileges active" || bad "PostgreSQL no-new-privileges not active"
[[ "$security_state" == *'Seccomp:'$'\t''2'* ]] && ok "PostgreSQL seccomp filtering active" || bad "PostgreSQL seccomp filtering not active"
...
if git -C "$REPO_ROOT" ls-files | grep -Eq '(^|/)(secrets|credentials)/|runtime\.local\.env$|\.key$|\.pem$|\.secret$'; then
    bad "Git tracks a secret-like path"
else
    ok "Git secret-pattern check passed"
fi
